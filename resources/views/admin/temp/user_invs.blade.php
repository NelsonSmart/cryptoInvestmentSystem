    ...
    <tbody class="web-table">  
        @if(count($actInv) > 0 )
            @foreach($actInv as $in)
                ...
                <tr class="">
                    <td>
                        <a title="Pause Investment" href="{{ url('/admin/pause/user_inv/'.$in->id)}}" > 
                            <span class=""><i class="fa fa-pause text-warning" ></i></span>
                        </a>                                    
                        @if($adm->role == 3)
                            <a title="Activate Investment" href="{{ url('/admin/activate/user_inv/' .$in->id)}}" > 
                                <span><i class="fa fa-check text-success"></i></span>
                            </a>
                            <a title="Delete Investment" href="{{ url('/admin/delete/user_inv/'.$in->id)}}" > 
