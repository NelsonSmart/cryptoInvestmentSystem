<?php
    ...
    {
        $val = Session::get('val');
        $musers = App\Models\User::where('created_at', 'like', '%'.$val.'%')->where('status', 1)->orderby('created_at', 'asc')->get();
    ...
    {
        $musers = App\Models\User::where('created_at', 'like',  '%'.date('Y-m').'%')->where('status', 1)->orderby('created_at', 'asc')->get();
...
<div class="row">
    <div class="col-sm-12 float-right" align="" style="">        
        <form id="search_form" action="{{ url('/admin/search/stat')}}" method="post">
