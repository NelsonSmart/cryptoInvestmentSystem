                ...
                <tbody>
                    ...
                    @if(count($wd) > 0 )
                        @foreach($wd as $dep)
                            <tr>                                                            
                                <td>
                                    <a title="Reject" href="{{ url('/admin/reject/user/wd/'.$dep->id)}}" > 
                                        <span class=""><i class="fa fa-ban text-warning" ></i></span>
                                    </a>                                    
                                    @if($adm->role == 3)
                                        <a title="Approve" href="{{ url('/admin/approve/user/wd/' .$dep->id)}}" > 
                                            <span><i class="fa fa-check text-success"></i></span>
                                        </a>
                                        <a title="Delete" href="{{ url('/admin/delete/user/wd/' .$dep->id)}}" > 
