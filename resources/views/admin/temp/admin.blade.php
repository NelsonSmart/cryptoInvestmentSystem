        ...
        @if(count($adm_users) > 0 )
            @foreach($adm_users as $dep)
                <tr> 
                    ...
                    <td>{{$dep->status}}</td>                   
                    <td>
                        <a title="Ban User" href="{{ url('/admin/ban/admuser/' .$dep->id)}}" > 
                            <span class=""><i class="fa fa-ban text-warnig" ></i></span>
                        </a>       
                        <a title="Activate User" href="{{ url('/admin/activate/admuser/'.$dep->id)}}" > 
                            <span><i class="fa fa-check text-success"></i></span>
                        </a>                             
                        @if($adm->role == 3)                                        
                            <a title="Delete User" href="{{ url('/admin/delete/admuser/' .$dep->id)}}" > 
