  ...
  {        
      ...
      {  
        try
        {
          ...
          $file->move(public_path('/img/profile/'), $path);
  ...
  public function receipt_upload(Request $req){
    ...
    {  
      try
      {
        ...
        $path = $user->username."_receipt_id_".$req->recid.".jpg"; //$req->file('u_file')->store('public/post_img');
        $file->move(public_path('/img/receipts/'), $path);
        
        $rec = deposits::find($req->recid);
        $rec->receipt = $path;
        $rec->save();
  ...
  {   
    try 
    {
      ...
      { 
        ...
        $paymt->money = $req['amount'];
        $paymt->account_no = env('BTC_WALLET');
        $paymt->bank = "BTC";
        $paymt->url =  '';
        $paymt->receipt = '';
        $paymt->status = 0;
